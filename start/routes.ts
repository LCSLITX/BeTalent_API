...
// CLIENT ROUTES
router.get('/clients', async (ctx: HttpContext) => {
  return await ClientsController.index(ctx)
}).use(middleware.auth())

router.post('/client', async (ctx: HttpContext) => {
  return await ClientsController.store(ctx)
}).use(middleware.auth()).use(middleware.cpfParser())

router.get('/client/:id', async (ctx: HttpContext) => {
  return await ClientsController.show(ctx)
}).use(middleware.auth())

router.put('/client/:id', async (ctx: HttpContext) => {
  return await ClientsController.update(ctx)
}).use(middleware.auth()).use(middleware.cpfParser())

router.delete('/client/:id', async (ctx: HttpContext) => {
...
router.post('/product', async (ctx: HttpContext) => {
  return await ProductsController.store(ctx)
}).use(middleware.auth())

router.get('/product/:id', async (ctx: HttpContext) => {
  return await ProductsController.show(ctx)
}).use(middleware.auth())

router.put('/product/:id', async (ctx: HttpContext) => {
  return await ProductsController.update(ctx)
}).use(middleware.auth())

router.delete('/product/:id', async (ctx: HttpContext) => {
  return await ProductsController.destroy(ctx)
}).use(middleware.auth())


// SALE ROUTES
router.post('/sale', async (ctx: HttpContext) => {
  return await SalesController.store(ctx)
}).use(middleware.auth())

router.get('/sales', async (ctx: HttpContext) => {
